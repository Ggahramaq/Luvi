    ...
    try{
        if (newLvl === 0){
            await User.findOneAndUpdate(
                {guildId: guildId, userId: targetUserId},{level: newLvl+1, xp:(((newLvl) * (newLvl) * (newLvl)) * 25) },{upsert:true, new:true}
            ).then(() => interaction.editReply({ content: `Успешно поменяли ранг ${member.user.username} на ${newLvl+1}(нельзя поменять уровень на 0)`}))
        } else {
            if (newLvl === 1) {
            await User.findOneAndUpdate(
                {guildId: guildId, userId: targetUserId},{level: newLvl, xp:(((newLvl-1) * (newLvl-1) * (newLvl-1)) * 25) },{upsert:true, new:true}
            ).then(() => interaction.editReply({ content: `Успешно поменяли ранг ${member.user.username} на ${newLvl}`}))
        } else {
            await User.findOneAndUpdate(
                {guildId: guildId, userId: targetUserId},{level: newLvl, xp:(((newLvl-1) * (newLvl-1) * (newLvl-1)) * 25) + 20},{upsert:true, new:true}
            ).then(() => interaction.editReply({ content: `Успешно поменяли ранг ${member.user.username} на ${newLvl}`}))  
        }
    }
