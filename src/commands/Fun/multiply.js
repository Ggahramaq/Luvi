...
module.exports = {
    ...
    .setName('multiply')
    .setDescription(`Умножит любые числа`)
    ...
    async execute(client, interaction) {
        ...
        const third = parseFloat(interaction.options?.get('третье-число')?.value || 1);
        const fourth = parseFloat(interaction.options?.get('четвертое-число')?.value || 1);
        const fifth = parseFloat(interaction.options?.get('пятое-число')?.value || 1);
